  ...
  # load dataset ----
  er <- read.csv('22 03 03 Lake Erie Mastersheet.csv',
  ...
  # inverts
  excr.inverts <- excr %>% filter(Species.code == "QM") %>% 
    mutate(massnorm.P.excr = P.excretion.rate/Mass,
           massnorm.N.excr = N.excretion.rate/Mass,
           Log10.massnorm.N.excr = log10(massnorm.N.excr),
           Log10.massnorm.P.excr = log10(massnorm.P.excr))
  ...
  PexcrSp.p <- ggplot(excr.verts,
         aes(x = Species.code, y = Log10.massnorm.P.excr)) +
    geom_jitter(size = 3, alpha = .5, aes(color = Season),
               position = position_jitterdodge(jitter.width = 0.2)) +
    geom_boxplot(alpha = 0, size = 0.8, aes(color = Season)) +
    geom_jitter(data = excr.inverts %>%  filter(Log10.mass > -2.328827),
                position = position_jitterdodge(jitter.width = 0.2),
               aes(x = Species.code, y = Log10.massnorm.P.excr,
                                 color = Season), size = 3, alpha = .5) +
    geom_boxplot(data = excr.inverts %>%  filter(Log10.mass > -2.328827),
                 size = 0.8,
                 aes(x = Species.code, y = Log10.massnorm.P.excr,
                                   color = Season), alpha = 0) +
    labs(x = 'Species',
         y = expression(atop(Log10~mass-corrected, 
                             paste(P~excretion~(μg~P/g/h))))) +
    scale_colour_manual(labels = c("Fall", "Summer"),
                        values = c("#D16103", "goldenrod2")) +
  ...
  ggsave('preliminary figures for trends/N_P excretion sp.png', 
         width = 20, height = 14, 
         units = 'in', dpi = 600)
  
  # ..N excretion rate vs season ----
  NexcrSea.p <- ggplot(excr.verts,
                    aes(x = Season, y = Log10.massnorm.N.excr, 
                        color = Season)) +
    stat_halfeye(adjust = .5, width = .6, .width = 0,justification = -.3,
      alpha = .3, aes(fill = Season)) + 
    ...
    scale_colour_manual(values = c("#D16103", "goldenrod2")) +
    scale_fill_manual(values = c("#D16103", "goldenrod2")) +
    theme(axis.text = element_text(face = 'bold'),
          axis.line = element_line(size = 1),
          panel.grid = element_blank(),
          axis.text.x = element_blank(),
  ...
  PexcrSea.p <- ggplot(excr.verts,
                       aes(x = Season, y = Log10.massnorm.P.excr, 
                           color = Season)) +
    stat_halfeye(adjust = .5, width = .6, .width = 0,justification = -.3,
                 alpha = .3, aes(fill = Season)) + 
    geom_boxplot(width = .25, size = 0.8, outlier.shape = NA) +
  ...
  ggarrange(NexcrSea.p, PexcrSea.p, nrow = 2, heights = c(1,1),
            ...
            legend = 'none', align = 'v')
  ggsave('preliminary figures for trends/N_Pexcretion seasonxsp.png', 
         width = 16, height = 14, 
         units = 'in', dpi = 600)
