  ...
  # 
  # excr <- excr %>% mutate(massnorm.N.excr.sp = N.excretion.rate/
  ...
  species <- c('Brown Bullhead', 'Goldfish', 'Gizzard shad', 'Largemouth bass', 
               'Logperch', 'Northern Pike', 'Roung goby', 'Walleye', 'White perch',
  ...
  NexcrSp.p <- ggplot(excr.verts %>% filter(Species.code != "CTL1", 
                                          Species.code != "CTL2",
                                          Species.code != "CTL3", 
                                          Species.code != "CTL4",
                                          Species.code != "CTL5", 
    ...
    geom_jitter(size = 3, alpha = .5, width = .2, aes(color = Season)) +
    geom_boxplot(alpha = 0, aes(color = Season)) +
    # geom_point(excr1, size = 3, alpha = .5) +
    # geom_boxplot(excr1, alpha = 0) +
    # geom_point(excr2, size = 3, alpha = .5) +
  ...
  Nexcr.p <- ggplot(excr.verts %>% filter(Species.code != "CTL1", 
    ...
    ggdist::stat_halfeye(
      ...
      .width = 0, 
      justification = -.3, 
      point_colour = NA, aes(color = Season)) + 
    geom_boxplot(
      width = .25, 
      outlier.shape = NA,
      aes(color = Season)
  ...
  ggsave('preliminary figures for trends/P excretion vs season&species.png', 
         width = 20, height = 14, 
         units = 'cm', dpi = 300)
  ggsave('preliminary figures for trends/P excretion vs season.png', 
         width = 20, height = 14, 
         units = 'cm', dpi = 300)
