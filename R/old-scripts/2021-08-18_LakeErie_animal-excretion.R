  ...
  # load dataset ----
  er <- read.csv('21 08 14 Lake Erie Mastersheet.csv',
  ...
  ggplot(excr %>% filter(Species.code != 'QM'), 
    ...
    geom_point()
  model <- lm(Log10.P.excretion.rate ~ Log10.mass, data = excr)
  model$coefficients
  ...
  excr <- excr %>% 
    mutate(massnorm.P.excr = P.excretion.rate/(Mass^0.61))
